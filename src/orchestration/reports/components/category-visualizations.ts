...
export function generateCategoryBenchmarkBars(
  ...
  const height = categoryAnalyses.length * (barHeight + 8) + 40;
    ...
    return `
      <g>
        ...
        <text x="${labelWidth - 8}" y="${y + barHeight / 2 + 4}"
              text-anchor="end" font-size="11" fill="${COLORS.bizNavy}"
        ...
        <!-- Company score bar -->
        <rect x="${labelWidth}" y="${y}" width="${companyWidth}" height="${barHeight}"
              fill="${color}" rx="4" />

        <!-- Benchmark line -->
        <line x1="${benchX}" y1="${y - 2}" x2="${benchX}" y2="${y + barHeight + 2}"
              stroke="${COLORS.bizNavy}" stroke-width="2" stroke-dasharray="4,2" />

        <!-- Score label -->
        <text x="${labelWidth + companyWidth + 8}" y="${y + barHeight / 2 + 4}"
              font-size="11" font-weight="bold" fill="${COLORS.bizNavy}">
          ${ca.overallScore}
        </text>

        <!-- Gap indicator -->
        ${showGap ? `
        <text x="${width - 40}" y="${y + barHeight / 2 + 4}"
              font-size="10" fill="${gap >= 0 ? COLORS.good : COLORS.needsImprovement}"
              text-anchor="end">
          ${gap >= 0 ? '+' : ''}${gap}
        </text>
        ` : ''}
